...
Usage:
    python adapters/python/scripts/validate_handoff.py --file workspace/stages/01_requirements/output/handoff_to_ba.json
    python adapters/python/scripts/validate_handoff.py --dir workspace/stages --strict
"""
...
SCHEMA_PATH = REPO_ROOT / "core/schemas/handoff.schema.json"
WORKFLOW_PATH = REPO_ROOT / "core/config/workflow.yml"
...
def main() -> int:
    ...
    schema = _load_json(_resolve_repo_path(args.schema))
    policy = _load_workflow_policy()
    target_path = _resolve_repo_path(args.file if args.file else args.dir)
